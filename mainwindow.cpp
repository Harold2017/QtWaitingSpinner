...
MainWindow::MainWindow(QWidget* parent): QMainWindow(parent), ui(new Ui::MainWindow), m_spinner(NULL)
{
    ui->setupUi(this);
   ...
   * see "launchBlockingSpinner" below*/
    ...
    connect(ui->plainTextEdit, &QPlainTextEdit::textChanged, [this]() { setText(ui->plainTextEdit->toPlainText()); });
...
void MainWindow::showColourDialog()
{
    ...
    connect(colorDialog, SIGNAL(currentColorChanged(QColor)), this, SLOT(setColor(QColor)));
...
void MainWindow::showTextColourDialog()
{
    QColorDialog* colorDialog = new QColorDialog(Qt::black, this);
    connect(colorDialog, &QColorDialog::currentColorChanged, [this](QColor color) { setTextColor(color); });
    connect(colorDialog, &QColorDialog::colorSelected, [this](QColor color) { setTextColor(color); });
    colorDialog->show();
}
...
void MainWindow::launchBlockingSpinner()
{
   ...
   * both are running at the same time. */
    ...
    WaitingSpinnerWidget* blockingSpinner = new WaitingSpinnerWidget(Qt::ApplicationModal, this, true);
...
void MainWindow::setLineLength(int length)
{
    m_spinner->setLineLength(length);
}

/*----------------------------------------------------------------------------*/

void MainWindow::setLineWidth(int width)
{
    m_spinner->setLineWidth(width);
}
...
void MainWindow::setText(QString const& text)
{
    m_spinner->setText(text);
}

/*----------------------------------------------------------------------------*/

void MainWindow::setTextColor(QColor color)
{
    m_spinner->setTextColor(color);
}
